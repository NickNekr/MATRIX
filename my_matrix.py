...
class Matrix(object):
    ...
    def solve(self, data):
        ans = []
        matrix_slau = [i + [j] for i, j in zip(self.lines, data)]
        ...
        for i in range(len(matrix_slau) - 1, 0, -1):
            for j in range(0, i):
                matrix_slau[j] = \
                    [j + i for j, i in zip(
                        list(map(
                            lambda x: x * (-1) * matrix_slau[j][i],
                            matrix_slau[i])),
                        matrix_slau[j])]
        del matrix_slau[len(matrix_slau) - matrix_slau.count(zero):]
        for i in matrix_slau:
            ans.append(i.pop())
        if matrix_slau == [[0 if i != j else 1 for i in range(
                len(matrix_slau[0]))] for j in range(len(matrix_slau))]:
            return ans
        else:
            raise Exception
